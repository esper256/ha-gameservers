...
- Detect ready, version, join, and leave from Factorio logs.

## 1.0.5

- Log watching stays conservative until patterns are proven.

## 1.0.4

- Fix updates leaving leftover files that crash the server.

## 1.0.3

- **Space Age DLC** option (default off). Saves made with it on cannot simply turn it off — use NEW WORLD.

## 1.0.2

- **Release channel**: Stable (default) or Experimental.
- Factorio no longer logs a scary stdin EOF after a successful host.

## 1.0.1

- Install from Wube’s free headless package (not Steam). Factorio.com login is only for public listing.

## 1.0.0

- First Factorio dedicated-server app.
- UDP **34197**. World saves as `.zip`. Player join/leave from console logs.
